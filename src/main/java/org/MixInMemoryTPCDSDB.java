...
public class MixInMemoryTPCDSDB {
    Connection conn;
    ...
    public MixInMemoryTPCDSDB(Config config) throws IOException, SQLException {
        sizeFactor = config.sizeFactor;
        conn = DriverManager.getConnection("jdbc:duckdb:" + config.readFolder + "tmp/mix_in_memory.db");
        String synthRootFolderName = config.readFolder + "input/data/tpc-ds/" + sizeFactor;
    ...
    private void mixFiles(String residuFile, String insertFile, String finalStreamFile, double perc) throws IOException, SQLException {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(finalStreamFile))) {
            writer.write("id,c_salutation,c_first_name,c_last_name,c_birth_country,c_current_addr_sk,sign\n");
...
//                    int id = rs.getInt("id");
//                    String c_salutation = rs.getString("c_salutation");
//                    String c_first_name = rs.getString("c_first_name");
//                    String c_last_name = rs.getString("c_last_name");
//                    String c_birth_country = rs.getString("c_birth_country");
//                    int c_current_addr_sk = rs.getInt("c_current_addr_sk");
//                    int sign = rs.getInt("sign");
//                    String insertSQL = "INSERT INTO inserts (id, c_salutation, c_first_name, c_last_name, c_birth_country, c_current_addr_sk, sign) " +
//                            "VALUES (?, ?, ?, ?, ?, ?, ?)";
//                    try (PreparedStatement insertStmt = conn.prepareStatement(insertSQL)) {
//                        insertStmt.setInt(1, id);
//                        insertStmt.setString(2, c_salutation);
//                        insertStmt.setString(3, c_first_name);
//                        insertStmt.setString(4, c_last_name);
//                        insertStmt.setString(5, c_birth_country);
//                        insertStmt.setInt(6, c_current_addr_sk);
//                        insertStmt.setInt(7, sign);
...
//                    "sign INTEGER" +
//                    ")");
            stmt.execute(String.format("COPY inserts FROM '%s' (DELIMITER ',', HEADER TRUE, NULL '')", insertFile));
    ...
    private void prepareStatement() throws SQLException {
        String sql = "SELECT id, c_salutation, c_first_name, c_last_name, c_birth_country, c_current_addr_sk, sign FROM inserts WHERE id = ?";
    ...
    private void batchFetchInsertRecords(List<InsertRef> ids) throws SQLException {
        ...
        String sql = "SELECT id, c_salutation, c_first_name, c_last_name, c_birth_country, c_current_addr_sk, sign " +
